...
class User implements \JsonSerializable {
	...
	public static function getUserByHash(\PDO $pdo, string $userHash){
		...
		if(empty($userHash) === true){
			throw (new \PDOException("userHash is invalid"));
		}

		// create query template
		$query = "SELECT userId, userName, userHash, userSalt, userAddress, userEmail FROM user WHERE userHash LIKE :userHash";
		$statement = $pdo->prepare($query);

		// bind userHash to placeholder in template
		$userHash = "%$userHash%";
